...
class fusion(coefficient):
    def __init__(self, params, sim, x_data, y_data, sigma_dir) -> None:
        # self.sigma = sigma_dir
        # for i in range(self.sim['num_time_intervals']):
        #     if i==0:
        #         tmp = x_data[:,:,i]
        #     else:
        #         tmp = torch.cat((tmp,x_data[:,:,i]),axis=0)
        self.x = x_data.clone().detach()
        self.val = sigmanet(params,sim)
        self.y_true = y_data.clone().detach()
        self.loss_epoch = []
        super(fusion, self).__init__(params)
    
    def train(self,lr=1e-2,delta_loss=1e-9,max_num_epochs=3000):
    ...
    def time_display(self, t_0, t_1):
        print(r'Training this epoch takes {:,} ms. So far: {:,} ms in training.'.format(round(1000*(time()-t_1),2),round(1000*(time()-t_0),2)))
