...
def main(argv):
    ...
    for i in range(sim_params['num_time_intervals']):
        if i == 0:
            x = semi.x[:,:,i]
            y = sigma(semi.x[:,:,i]).squeeze(-1)
        else:
            x = torch.cat((x,semi.x[:,:,i].squeeze(-1)),axis=0)
            y = torch.cat((y,sigma(semi.x[:,:,i]).squeeze(-1)),axis=0)
    ...
    output_dict[0]['sigma'] = {'min':y[:,0,0].min().clone().detach().numpy().item(),
                            'mean':y[:,0,0].mean().clone().detach().numpy().item(),
                            'median':y[:,0,0].median().clone().detach().numpy().item(),
                            'max':y[:,0,0].max().clone().detach().numpy().item(),
                            'std':y[:,0,0].std().clone().detach().numpy().item()
                            }
    
    df['ite1'] = y[:,0,0].clone().detach().numpy()
    ...
    for j in range(1,num_ite+1):
        ...
        for i in range(sim_params['num_time_intervals']):
            if i == 0:
                ...
                y = torch.cat((y,sigma(semi.x[:,:,i]).squeeze(-1)),axis=0)
        output_dict[j] = {}
        output_dict[j]['ell'] = {'min':y[:,0,0].min().clone().detach().numpy().item(),
                                'mean':y[:,0,0].mean().clone().detach().numpy().item(),
                                'median':y[:,0,0].median().clone().detach().numpy().item(),
                                'max':y[:,0,0].max().clone().detach().numpy().item(),
                                'std':y[:,0,0].std().clone().detach().numpy().item(),
        ...
        for i in range(sim_params['num_time_intervals']):
            if i == 0:
                ...
                x = torch.cat((x,semi.x[:,:,i].squeeze(-1)),axis=0)
                y = torch.cat((y,sigma(semi.x[:,:,i]).squeeze(-1)),axis=0)
        output_dict[j]['sigma'] = {'min':y[:,0,0].min().clone().detach().numpy().item(),
                                'mean':y[:,0,0].mean().clone().detach().numpy().item(),
                                'median':y[:,0,0].median().clone().detach().numpy().item(),
                                'max':y[:,0,0].max().clone().detach().numpy().item(),
                                'std':y[:,0,0].std().clone().detach().numpy().item()
        ...
        df["ite"+str(j+1)] = y[:,0,0].clone().detach().numpy()
        # fusing for stability of sigma
        # print(x.shape,y[:,0,0].shape)
        sigma = cf.fusion(pde_params, sim_params, x, y[:,0,0].unsqueeze(-1), sigma)
        sigma.train()
        for i in range(sim_params['num_time_intervals']):
            if i == 0:
                y = sigma(semi.x[:,:,i]).squeeze(-1)
                ...
                y = torch.cat((y,sigma(semi.x[:,:,i]).squeeze(-1)),axis=0)
        output_dict[j]['fused'] = {'min':y[:,0,0].min().clone().detach().numpy().item(),
                                'mean':y[:,0,0].mean().clone().detach().numpy().item(),
                                'median':y[:,0,0].median().clone().detach().numpy().item(),
                                'max':y[:,0,0].max().clone().detach().numpy().item(),
                                'std':y[:,0,0].std().clone().detach().numpy().item()
                                }
        df["fused_ite"+str(j+1)] = y[:,0,0].clone().detach().numpy()
