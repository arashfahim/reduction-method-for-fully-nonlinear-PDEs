...
class data_gen(object):
    def __init__(self,sigma,mu,pde,sim):
        ...
        self.dw = iid[0:self.num_samples*self.dim*n].reshape([self.num_samples,self.dim,n]).to(device)* torch.sqrt(self.dt)#you can make randomness a universal variable if 
        self.x = torch.zeros((self.num_samples,self.dim+1,n+1))
        
        
        self.x[:,1:,0]= a+(b-a)*torch.rand(self.num_samples,self.dim).to(device)  
        self.sigmadw = torch.zeros((self.num_samples,self.dim,n)).to(device)
        for i in range(n):
